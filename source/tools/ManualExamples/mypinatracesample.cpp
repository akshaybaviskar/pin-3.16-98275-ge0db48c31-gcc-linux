...
ADDRINT PIN_FAST_ANALYSIS_CALL IsRecordToWarmup()
{
  // cout<<"samp = "<<samplecount<<" ret "<<(samplecount == 5)<<";";
  return (prevsamplecount == 1) && (samplecount == 2);
}

ADDRINT PIN_FAST_ANALYSIS_CALL IsWarmupToRecord()
{
  // cout<<"samp = "<<samplecount<<" ret "<<(samplecount == 5)<<";";
...
VOID Instruction(INS ins, VOID *v)
{
 ...
 //  INS_InsertThenCall(ins, IPOINT_BEFORE, (AFUNPTR)InsertMem,  IARG_CALL_ORDER, CALL_ORDER_DEFAULT + 10,IARG_END);
   ...
   INS_InsertIfCall(ins, IPOINT_BEFORE, (AFUNPTR)IsRecordToWarmup,  IARG_CALL_ORDER, CALL_ORDER_DEFAULT + 10,IARG_END);
   INS_InsertThenCall(ins, IPOINT_BEFORE, (AFUNPTR)InsertRecordToWarmup,  IARG_CALL_ORDER, CALL_ORDER_DEFAULT + 10,IARG_END);

   INS_InsertIfCall(ins, IPOINT_BEFORE, (AFUNPTR)IsWarmupToRecord,  IARG_CALL_ORDER, CALL_ORDER_DEFAULT + 20,IARG_END);
   INS_InsertThenCall(ins, IPOINT_BEFORE, (AFUNPTR)InsertWarmupToRecord,  IARG_CALL_ORDER, CALL_ORDER_DEFAULT + 20,IARG_END);
...
*/
    ...
    for (UINT32 memOp = 0; memOp < memOperands; memOp++)
    {
        ...
        {
            INS_InsertIfPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)IsSample,  IARG_CALL_ORDER, CALL_ORDER_DEFAULT + 30,IARG_END);
            INS_InsertThenCall(
                ins, IPOINT_BEFORE, (AFUNPTR)RecordMemRead, IARG_CALL_ORDER, CALL_ORDER_DEFAULT + 30,
                IARG_INST_PTR,
